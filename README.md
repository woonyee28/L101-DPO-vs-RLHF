...
A modular framework for aligning and evaluating Large Language Models (specifically Pythia) using Reinforcement Learning from Human Feedback (RLHF) and Direct Preference Optimisation (DPO). HELM is used to collect results for multiple bias amplification benchmarks (BBQ, Disinformation, and TruthfulQA).
